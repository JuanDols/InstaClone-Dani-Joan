...
<body>
    ...
    if (isset($_SESSION["logged_id"])) {
        if ($_SESSION["logged_id"] == $_GET["userid"]) {
            ...
            //comprobar si al usuario de la sesion ha sido aceptado como seguidor
            $sql = "SELECT * FROM followers WHERE follower_id = " . $_SESSION["logged_id"] . " AND following_id = " . $currentuserid;
            $result = execQuery(getDbConnection(), $sql);
            if (mysqli_num_rows($result) > 0) {
                ...
                //el perfil visitado no ha aceptado la solicitud de seguimiento del usuario de la sesion, por tanto se redirige a su feed
